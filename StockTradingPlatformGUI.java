 ...
 * Updated: Added Price History Chart
 */
public class StockTradingPlatformGUI extends JFrame {
    ...
    Map<String, LinkedList<Double>> priceHistory = new HashMap<>();
    ...
    public StockTradingPlatformGUI() {
        ...
        chartHolder.setPreferredSize(new Dimension(0, 160));
        chartHolder.add(buildChart(selectedSymbol), BorderLayout.CENTER);
        center.add(chartHolder, BorderLayout.SOUTH);
    ...
    void initMarket() {
        ...
        for (String sym : market.keySet()) {
            LinkedList<Double> h = new LinkedList<>();
            h.add(market.get(sym).price);
            priceHistory.put(sym, h);
        }
    }
    ...
    JPanel buildMarketPanel() {
        ...
        mkt.getSelectionModel().addListSelectionListener(ev -> {
            int row = mkt.getSelectedRow();
            if (row >= 0 && row < market.size()) {
                selectedSymbol = (String) marketModel.getValueAt(row, 0);
                updateChart();
            }
        });
    ...
    // ── Price History Chart ───────────────────────────────────────────────
    JPanel buildChart(String symbol) {
        LinkedList<Double> hist = priceHistory.getOrDefault(symbol, new LinkedList<>());
        ...
        JPanel chart = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                int w = getWidth(), h = getHeight(), pad = 36;
                g2.setColor(CARD);
                g2.fillRect(0, 0, w, h);
                ...
                g2.drawString("📈 " + symbol + " — Price History (last 20 ticks)  |  Click a stock row to switch", pad, 16);

                if (hist.size() < 2) {
                    g2.setColor(SUBTEXT);
                    g2.setFont(BODY_F);
                    g2.drawString("Not enough data. Click '🔄 Simulate Market' a few times to generate history.", pad, h / 2);
                    return;
                }

                Double[] prices = hist.toArray(new Double[0]);
                double min = Arrays.stream(prices).mapToDouble(x->x).min().orElse(0);
                double max = Arrays.stream(prices).mapToDouble(x->x).max().orElse(1);
                ...
                int[] xPts = new int[prices.length + 2];
                int[] yPts = new int[prices.length + 2];
                boolean isUp = prices[prices.length - 1] >= prices[0];
                for (int i = 0; i < prices.length; i++) {
                    xPts[i] = pad + (chartW * i / (prices.length - 1));
                    yPts[i] = 24 + (int) ((max - prices[i]) / range * chartH);
                }
                xPts[prices.length]     = xPts[prices.length - 1];
                yPts[prices.length]     = 24 + chartH;
                xPts[prices.length + 1] = pad;
                yPts[prices.length + 1] = 24 + chartH;
                Color lineColor = isUp ? GREEN : RED;
                g2.setColor(new Color(lineColor.getRed(), lineColor.getGreen(), lineColor.getBlue(), 30));
                g2.fillPolygon(xPts, yPts, prices.length + 2);

                // Price line
                g2.setStroke(new BasicStroke(2f));
                g2.setColor(lineColor);
                for (int i = 1; i < prices.length; i++) {
                    int x1 = pad + (chartW * (i-1) / (prices.length-1));
                    int y1 = 24 + (int) ((max - prices[i-1]) / range * chartH);
                    int x2 = pad + (chartW * i / (prices.length-1));
                    int y2 = 24 + (int) ((max - prices[i]) / range * chartH);
                    g2.drawLine(x1, y1, x2, y2);
                }

                // Latest price dot
                int lx = pad + chartW;
                int ly = 24 + (int) ((max - prices[prices.length-1]) / range * chartH);
                g2.setColor(lineColor);
                g2.fillOval(lx - 4, ly - 4, 8, 8);
                g2.setColor(TEXT);
                g2.setFont(new Font("Segoe UI", Font.BOLD, 10));
                g2.drawString(String.format("$%.2f", prices[prices.length-1]), lx + 6, ly + 4);
            }
        };
        chart.setBackground(CARD);
        chart.setBorder(BorderFactory.createLineBorder(BORDER));
        return chart;
    }
    ...
    void simulateMarket() {
        for (Stock s : market.values()) {
            ...
            LinkedList<Double> hist = priceHistory.get(s.symbol);
            hist.add(s.price);
